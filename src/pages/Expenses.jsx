...
const Expenses = () => {
  ...
  const { yearlyData, pieData, recentExpenses, monthlyTotal, topCategory } = useMemo(() => {
    ...
    const pData = Object.keys(catMap).map(c => ({ name: c, value: catMap[c] }));
    const topCat = pData.sort((a, b) => b.value - a.value)[0]?.name || 'N/A';

    return { yearlyData: yData, pieData: pData, recentExpenses: recent, monthlyTotal: monthSum, topCategory: topCat };
  }, [appData.expenses]);
  ...
  const handleAddSubmit = (e) => {
    ...
    setFormData({ merchant: '', category: 'Food', amount: '', date: new Date().toISOString().split('T')[0], paymentMethod: 'Credit Card' });
    ...
    <div className="p-4 md:p-8 w-full max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-2">
        <div>
          ...
          <p className="text-neutral opacity-70">Monitor your spending and outflows</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-base-200 p-6 rounded-2xl shadow-lg border border-base-300">
          <p className="text-sm font-medium text-neutral opacity-70 mb-1">1-Month Spend (April)</p>
          <p className="text-3xl font-bold text-[#ef4444]">${monthlyTotal.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
        </div>
        <div className="bg-base-200 p-6 rounded-2xl shadow-lg border border-base-300">
          <p className="text-sm font-medium text-neutral opacity-70 mb-1">Top Spending Sector</p>
          <p className="text-3xl font-bold text-[#f97316]">{topCategory}</p>
        </div>
        <div className="bg-base-200 p-6 rounded-2xl shadow-lg border border-base-300">
          <p className="text-sm font-medium text-neutral opacity-70 mb-1">Total Expenses Logged</p>
          <p className="text-3xl font-bold text-[#8b5cf6]">{appData.expenses.length}</p>
        </div>
      </div>
            ...
            <tbody>
              {recentExpenses.map((exp) => (
                <tr key={exp._id} className="hover border-b border-base-300/50">
                  <td className="whitespace-nowrap">{new Date(exp.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</td>
                  <td className="font-medium">{exp.merchant}</td>
                  <td>
                    <span className="badge badge-sm bg-transparent border border-neutral text-neutral opacity-80">{exp.category}</span>
                  </td>
                  <td className="text-right font-bold text-[#ef4444]">-${exp.amount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                  
                  {auth.role === 'admin' && (
                    <td className="text-right space-x-2">
                      <button onClick={() => openEditModal(exp)} className="btn btn-xs btn-outline btn-info">Edit</button>
                      <button onClick={() => handleDelete(exp._id)} className="btn btn-xs btn-outline btn-error">Del</button>
                    </td>
                  )}
                </tr>
              ))}
      ...
      {auth.role === 'admin' && isAddModalOpen && (
        <dialog open className="modal bg-black/60 backdrop-blur-sm">
          <div className="modal-box bg-base-200 border border-base-300">
            <h3 className="font-bold text-xl mb-6 text-neutral">Log New Expense</h3>
            
            <form onSubmit={handleAddSubmit} className="space-y-4">
              <div>
                <label className="label text-sm font-medium text-neutral">Merchant / Description</label>
                <input 
                  type="text" 
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="label text-sm font-medium text-neutral">Category</label>
                  <select 
                ...
                <div>
                  <label className="label text-sm font-medium text-neutral">Date</label>
                  <input 
                    type="date" 
                    value={formData.date}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="label text-sm font-medium text-neutral">Amount ($)</label>
                  <input 
                    type="number" 
                    step="0.01"
                ...
                <div>
                  <label className="label text-sm font-medium text-neutral">Payment Method</label>
                  <select 
              ...
              <div className="modal-action mt-6">
                ...
                <button type="submit" className="btn bg-[#ef4444] hover:bg-[#dc2626] border-none text-white">Log Expense</button>
              </div>
            </form>
          </div>
        </dialog>
