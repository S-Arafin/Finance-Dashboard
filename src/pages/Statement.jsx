...
const Statement = () => {
      ...
      const updatedIncome = appData.income.map((item) =>
        item._id === updatedTransaction._id
          ? {
              ...
              netAmount: parseFloat(updatedTransaction.amount),
            }
          : item,
      );
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute(
      "download",
      `financial_statement_${new Date().toISOString().split("T")[0]}.csv`,
    );
    ...
    <div className="p-4 md:p-8 w-full max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-4 mb-6">
        <div className="w-full lg:w-auto">
          ...
          <p className="text-neutral opacity-70 mb-4 lg:mb-0">
        ...
        <div className="flex flex-col sm:flex-row items-end sm:items-center gap-3 w-full lg:w-auto">
          ...
          {auth.role === "admin" && (
            <span className="badge badge-error badge-outline font-bold hidden xl:flex">
          ...
          <div className="relative w-full sm:w-80">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-neutral opacity-50"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
      ...
      <div className="bg-base-200 rounded-2xl shadow-lg border border-base-300 overflow-hidden">
            ...
            <tbody>
              {currentItems.length > 0 ? (
                currentItems.map((tx) => (
                  <tr
                    key={tx._id}
                    className="hover text-neutral border-b border-base-300/50"
                  >
                    ...
                    <td className="whitespace-nowrap">
                      <span
                        className={`badge badge-sm bg-transparent border ${tx.type === "Income" ? "border-primary text-primary" : "border-neutral text-neutral opacity-80"}`}
                      >
                        {tx.type}
                      </span>
                    </td>
                    <td className="font-medium min-w-[150px]">
                      {tx.description}
                      <br />
                    ...
                    <td
                      className={`font-bold whitespace-nowrap ${tx.type === "Income" ? "text-primary" : "text-neutral"}`}
                    >
                      {tx.type === "Income" ? "+" : "-"}$
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between p-4 border-t border-base-300 bg-base-100 gap-4">
          <span className="text-sm text-neutral text-center sm:text-left">
            Showing{" "}
          ...
          <div className="join">
            <button
              ...
              className="join-item btn btn-sm border-base-300 text-neutral bg-base-200 hover:bg-base-300"
            >
              «
            </button>
            <button className="join-item btn btn-sm no-animation border-base-300 text-neutral bg-base-100">
